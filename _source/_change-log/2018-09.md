...
The following changes are available starting February 28, 2018 in preview orgs and starting March 5, 2018 in production orgs:

| [User Consent for OAuth 2.0 and OpenID Connect Flows in Early Availability](#user-consent-for-oauth-20-and-openid-connect-flows-in-early-availability)
| [Sessions API Supports HTTP Header Prefer](#sessions-api-supports-http-header-prefer)
| [Improved Response Mode](#improved-response-mode)
| [Change to `/authorize` Response for `prompt`](#change-to-authorize-response-for-prompt)
| [Changes to Error Messages from User Creation Requests](#changes-to-error-messages-from-user-creation-requests)
| [Improved System Log Behavior for Date Queries](#improved-system-log-behavior-for-date-queries)
| [System Log Message Changes](#system-log-message-changes)
| [Bug Fixes](#bug-fixes)
...
To configure an authorization or authentication flow:

1. Verify that you have the API Access Management feature enabled, and request that User Consent also be enabled.
2. Create an app via the Apps API with the appropriate values for `tos_uri`, `policy_uri`, and `consent_method`. [Doc](/docs/api/resources/apps.html#settings-7). You can also configure an existing app in the Admin user interface: **Applications > [Application Name] > General > User Consent**.
3. Ensure that your authentication or authorization flow is configured properly. The combination of `prompt` in the `/authorize` request, `consent_method` set on the app in the previous step, and `consent`, a property set on scopes, controls whether a user consent window is displayed during the authentication flow. [Details](/docs/api/resources/apps.html#settings-7)
...
### Improved Response Mode

For [the `form_post` response mode](/docs/api/resources/oidc.html#parameter-details), we have reduced the HTML content returned in an OpenID Connect or OAuth 2.0 request. Now the response is only a form containing the requested tokens (access token, ID token, or both) and JavaScript to post the form. <!-- OKTA-96521 -->

### Change to `/authorize` Response for `prompt`

If you set `prompt=none` for a request on `/v1/authorize` and the maximum age before sign-in is required (`max_age`) is exceeded, an error is returned. This ensures the safest possible result when [these two settings contradict each other](/docs/api/resources/oidc#parameter-details). This applies `/v1/authorize` whether or not you specify an authorization server.
...
### System Log Message Changes

#### Simplified `/events` Messages from [`/v1/authorize`](/docs/api/resources/oidc.html#authorize):
...
Details about the nature of the failure are in included, so no information has been lost with this simplification.

#### Simplified `/events` Messages from [`/v1/token`](/docs/api/resources/oidc.html#token)

The existing message `app.oauth2.as.authorize.token.grant_failure` replaces
these messages:

* `app.oauth2.as.token.grant.warn_failure`
* `app.oauth2.as.token.grant.scope_denied_failure`

#### Simplified `/events` and `/logs` Messages from Token Generation
...
#### Simplified `/logs` Messages from Token Generation
...
The following bugs have been fixed and are expected in preview orgs February 14, 2018 and production orgs starting February 27, 2018.
