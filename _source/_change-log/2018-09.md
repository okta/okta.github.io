...
2. Create an app via the Apps API with the appropriate values for `tos_uri`, `policy_uri`, and `consent_method`. [Doc](/docs/api/resources/apps#settings-7). You can also configure an existing app in the Admin user interface: **Applications > [Application Name] > General > User Consent**.
...
#### Simplified `/events` Messages from [`/v1/authorize`](/docs/api/resources/oidc#authorize):
...
#### Simplified `/events` Messages from [`/v1/token`](/docs/api/resources/oidc#token)
