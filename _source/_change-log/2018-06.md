...
Secure your APIs with API Access Management, Okta’s implementation of the OAuth 2.0 authorization framework. API Access Management uses the Okta Identity platform to enable powerful control over access to your APIs. API Access Management can be controlled through Okta’s user interface as well as a rich set of APIs for client, user, and policy management.
...
To change the role assigned to a user, use [the Administrator Roles API](/docs/api/resources/roles) or visit **Security > Administrators** in the Okta user interface. <!--OKTA-107617-->
