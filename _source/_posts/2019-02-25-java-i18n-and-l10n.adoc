---
layout: blog_post
title: "Java i18n and Localization (i18n and l10n)"
author: mraible
...
There are a number of acronyms related to internationalization and localization of an application. Most of them are long words, so their abbreviations are often `{first letter + number of letters + last letter}`. For example:
...
The first two are the most important for this article. What is i18n and l18n? Internationalization is the process of making your application _capable_ of rendering its text in multiple languages. Localization means your application has been coded in such a way that it meets language, cultural, or other requirements of a particular locale. These requirements can include formats for date, time, and currency, as well as symbols, icons and colors, among many other things. i18n enables l10n.
...
If you see any error like the one below, it's because you're using a Java version < 11. https://openjdk.java.net/jeps/330[JEP 330] is an enhancement in Java 11 that allows you to run a single file of Java source code, without compiling it.
...
Setup your environment to use the latest version of OpenJDK with the command below:
...
Open the project in your favorite IDE and create `HelloController.java` in `src/main/java/com/example/demo`.
...
This is a nice setup, but you might want to allow users to set their own language. You might've seen this on websites in the wild, where they have a flag that you can click on to change to that country's language. To make this possible in Spring Boot, create a `MvcConfigurer` class alongside your `HelloController`.
...
If you'd like to modify your Thymeleaf templates (and message bundles) and see those changes immediately when you refresh your browser, you can add Spring Boot's https://docs.spring.io/spring-boot/docs/current/reference/html/using-boot-devtools.html[Developer Tools] to your `pom.xml`.
...
This turns off https://docs.spring.io/spring-boot/docs/current/reference/html/using-boot-devtools.html#using-boot-devtools-property-defaults[caching static resources automatically]. Note that changing static files will not restart your server automatically, you need to install a https://docs.spring.io/spring-boot/docs/current/reference/html/using-boot-devtools.html#using-boot-devtools-livereload[LiveReload browser extension] to see changes immediately.

// NOTE: This doesn't work, any ideas?

== Customizing the Language used by Okta's Sign-In Widget
...
If you already have an Okta Developer account, log in to it. If you don't, create one at https://developer.okta.com/signup-now[developer.okta.com/signup]. After you're logged in to your Okta dashboard, complete the following steps:
...
TIP: I recommend adding the above exports to a `.okta.env` file in the root of your project and adding `*.env` to `.gitignore`. Then run `source .okta.env` before you run your app.
...
Restart the custom-login app and navigate to `http://localhost:8080/?lang=en`. If you click the login button, you'll see that the widget is still rendered in Spanish. To fix this, crack open `LoginController` and add `language` as model attribute. You might noticed this determines the language by injecting `LocaleResolver` in the constructor and using `localeResolver.resolveLocale(request)`.
...
=== Add i18n Bundles for Thymeleaf
...
To set the locale from the user's information in Okta, create a `OidcLocaleResolver` in the same directory as `MvcConfigurer`.
...
TIP: Baeldung's https://www.baeldung.com/spring-boot-internationalization[Guide to Internationalization in Spring Boot] was a useful resources when writing this post.

We like to write about Java and Spring Boot on this here blog. Here's a few of my favorites:
