...
== Use Schematics to add Authn to Your Angular App
...
How sweet is that?! Instead of having to 1) add `@okta/okta-angular` as a dependency, 2) create five files, and 3) update three files - our Schematics does it all for you in less than 30 seconds! 🎉😆
...
One cool thing about Schematics is they don't perform any direct actions on your filesystem. Instead, you specify what you'd like to do to a `Tree`. The `Tree` is a data structure with a set of files that already exists and a staging area (of files that will contain new/updated code). You can see in the code above that nothing is really happening, the test even proves the tree is empty!
...
Create a `files/src/app` directory to hold your templates in the `my-component/src/my-component` directory.

[source,shell]
----
mkdir -p files/src/app
----

Create an `app.component.ts` file in this directory and put the following code in it:
...
After creating these files, your templates should look as follows.

[source,shell]
----
$ tree src/my-component/files
src/my-component/files
└── src
    └── app
        ├── app.component.html
        └── app.component.ts
...
[source,shell]
----
ng new my-cool-app --routing --style css
----
...
Run the `ng g` command and everything should work this time.
...
Run `npm build` in your `my-component` project. Now you should be able to run `ng add my-component`. Pretty slick, eh?
...
cd my-secure-app
// create a SPA on Okta, copy settings
ng add @oktadev/schematics
----
...
We like to write about Angular on this here blog. Here's some of our recent Angular posts:
