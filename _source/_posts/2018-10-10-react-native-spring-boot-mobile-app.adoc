...
After overriding the default OIDC settings for Spring Security, you'll need to add `https://healthpoints.cfapps.io/login` as a Login redirect URI in your Okta OIDC application. You'll also want to add `https://healthpoints.cfapps.io` to your Logout redirect URIs if you want logout to work.
