...
To run your app on an Android emulator, you'll need to run a couple commands to allow it to communicate with your API and Keycloak.
...
Then run `react-native run-android`. If you don't have a phone plugged in or an Android Virtual Device (AVD) running, you'll see an error:
...
Run `react-native run-android` again. You might receive the following error:

----
Could not find com.android.tools:common:25.3.3
----

To fix this, open `node_modules/react-native-vector-icons/android/build.gradle` and change the `buildscript` block to be as follows.
...
Now you should be able to log in just like you did on iOS.
...
In the terminal where your Spring Boot app is running, kill the process, run `source ~/.okta.env` and run `./gradlew` again. You should be able to log in at `http://localhost:8080` in your browser.

=== Create a Native App with PKCE

Ignite JHipster leverages https://github.com/FormidableLabs/react-native-app-auth[React Native AppAuth], an SDK for communicating with OAuth 2.0 providers. It supports PKCE instead of a client secret, which is a more secure configuration. To use PKCE, you'll need to create a new Native application on Okta.
