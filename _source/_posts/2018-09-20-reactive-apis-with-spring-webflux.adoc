...
// Facebook or Twitter - would Instagram and Snapchat be better to appeal to the millenials?

// todo: figure out img tag for asciidoc, this spits out escaped HTML
{% raw %}{% img blog/spring-webflux-apis/starbuxman.jpg alt:"Josh Long" class:"BlogPost-avatar pull-right img-100px" %}{% endraw %} I collaborated on this post with https://twitter.com/starbuxman[Josh Long], fellow Java Champion, Spring Developer Advocate and all around great guy at Pivotal. I've been a longtime user of Spring, and Josh was the one who first showed me Spring Boot, at a Devoxx Belgium many moons ago. We've been good friends for a while now, sharing the same passion for Java, developers, and building awesome applications.

Most of the words in this post are from Josh (a.k.a. https://twitter.com/starbuxman[@starbuxman]). I added the part about securing your API with OIDC. Even though I show how to do it with Okta, it should work with any OIDC provider.
...
{% img blog/spring-webflux-apis/the-spring-initializr.png alt:"the Spring Initializr" width:"800" %}{: .center-image }
...
MongoDB is a document database, so the unit of interaction is a sparse document - think of it as a JSON stanza that gets persisted and is retrievable by a key, the document ID.
...
We only annotated one test because the rest are unremarkable and similar.
...
Open the `ws.html` page in the browser and then run that `create.sh` in your shell. I'm assuming you have `curl`. This is a trivial end-to-end and it's satisfying to see it all come together. That said, we should definitely test this. You know. Just in case..
...
== Secure Your Reactive API with OIDC
...
// are we already using Spring Boot 2.1? If not, we should so it's easy to update this post when it comes out.
...
* https://developer.okta.com/product/authentication/[Authenticate] and https://developer.okta.com/product/authorization/[authorize] your users
* Store data about your users
* Perform password-based and https://developer.okta.com/authentication-guide/social-login/[social login]
* Secure your application with https://developer.okta.com/use_cases/mfa/[multi-factor authentication]
* And much more! Check out our https://developer.okta.com/documentation/[product documentation]

Giddyup! [Register for a forever-free developer account](https://developer.okta.com/signup/), and when you're done, come on back so you can learn more about how to secure your reactive API!

=== Create an OIDC App in Okta

{uri-okta-login}[Log in] to your Okta Developer account (or https://developer.okta.com/signup/[sign up] if you don't one). Navigate to **Applications** > **Add Application**. Click **Web** and click **Next**. Give the app a name you'll remember, and specify `http://localhost:8080/login/oauth2/code/okta` as a Login redirect URI. Click **Done**, then click **Edit** to edit General Settings. Add `http://localhost:8080` as Logout redirect URIs, then click **Save**.
...
Restart your Spring Boot app, and try to access `http://localhost:8080`. You'll be redirected to Okta to login, then you'll be able to see your data.

// todo: show how to generate an access token, and access using curl or httpie
...
* reactive programming (the manifesto?)
* Spring WebFlux stuff (got a video or screencast?)
...
// todo: why no paragraph break after this list?
