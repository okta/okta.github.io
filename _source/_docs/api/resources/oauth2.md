...
The lifetime of Access Token can be configured in the [Access Policies](#access-policies). If the application that
issued the token is deactivated, the token is immediately and permanently invalidated. Reactivating the application
...
The lifetime of an Id Token is 1 hour. If the application that
issued the token is deactivated, the token is immediately and permanently invalidated. Reactivating the application
does not make the token valid again.
