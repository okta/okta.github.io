...
This is the default flow for new user registration with the Okta user interface.
...
The only permitted customization of the default profile is changing whether the `firstName` and `lastName` properties are nullable.  You can use the Profile Editor in the Okta user interface or the [Schemas API](schemas) to make schema modifications.

#### Custom Profile Properties

User profiles may be extended with custom properties but the property must first be added to the user profile schema before it can be referenced.  You can use the Profile Editor in the Okta User Interface or the [Schemas API](schemas) to manage schema extensions.
...
> Password policy requirements can be modified in the Okta user interface *(Security -> Policies)*
