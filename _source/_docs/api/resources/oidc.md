 ...
 * Each value for `response_mode` delivers different behavior:
    ...
    * The `Referrer-Policy` header is automatically included in the request for `fragment` or `query`, and is set to `Referrer-Policy: no-referrer`. However, 
    if the `response_mode` is `query`, the location specified in `redirect_uri` must set this header to prevent some codes from leaking.
