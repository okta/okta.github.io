...
For example, your app can support logging in with credentials from Facebook, Google, LinkedIn, Microsoft, or an enterprise IdP using SAML 2.0 or OpenID Connect protocol.
...
The `OAUTH2` and `OIDC` protocols support the `authorization` and `token` endpoints. Additionally, `OIDC` protocol supports the `userInfo` and `jwks` endpoints.
...
| Property      | Description                                                                               | DataType                                   | Nullable | Readonly |
| ------------- | ----------------------------------------------------------------------------------- | ----------------------------------------- | ----------- | ------------ |
| authorization | IdP Authorization Server (AS) endpoint to request consent from user and obtain an authorization code grant | [OAuth 2.0 Authorization Server Authorization Endpoint Object](#oauth-20-authorization-server-authorization-endpoint-object)| TRUE     | TRUE     |
| token         | IdP Authorization Server (AS) endpoint to exchange authorization code grant for an access token            | [OAuth 2.0 Authorization Server Token Endpoint Object](#oauth-20-authorization-server-token-endpoint-object)| TRUE     | TRUE     |
| userInfo      | Protected resource endpoint that returns claims about the the authenticated user                           | [OpenID Connect Userinfo Endpoint Object](#openid-connect-userinfo-endpoint-object)| TRUE     | TRUE     |
| jwks          | Endpoint where the signer of the keys publish its keys in a JWK Set                                        | [OpenID Connect JWKs Endpoint Object](#openid-connect-jwks-endpoint-object)| TRUE     | TRUE     |
...
|------------ | ------------------------------------------------- | ---------------------------- | ----------- | ------------ | -------------- | ---------------- |
| url         | URL of IdP Authorization Server (AS) authorization endpoint        | String     | TRUE      | TRUE        | 11        | [RFC 3986](https://tools.ietf.org/html/rfc3986) |
...
| Property    | Description                                       | DataType                     | Nullable    | Readonly     | MinLength      | Validation    |
|------------ | ------------------------------------------------- | ---------------------------- | ----------- | ------------ | -------------- | ---------------- |
| url         | URL of IdP Authorization Server (AS) token endpoint        | String     | TRUE      | TRUE        | 11        | [RFC 3986](https://tools.ietf.org/html/rfc3986) |
| binding     | HTTP binding used to send request to IdP Authorization Server (AS) endpoint | `HTTP-POST` or `HTTP-Redirect`   | TRUE     | TRUE     |           |           |
...
Endpoint for userinfo [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo).

The OpenID Connect endpoints are defined as part of the [IdP provider](#identity-provider-type) and are read only.

|------------ | ------------------------------------------------- | ---------------------------- | ----------- | ------------ | -------------- | ---------------- |
| Property    | Description                                       | DataType                     | Nullable    | Readonly     | MinLength      | Validation    |
|------------ | ------------------------------------------------- | ---------------------------- | ----------- | ------------ | -------------- | ---------------- |
| url         | URL of resource servers userinfo endpoint         | String     | TRUE      | TRUE        | 11        | [RFC 3986](https://tools.ietf.org/html/rfc3986) |
| binding     | HTTP binding used to send request to the protected resource | `HTTP-POST` or `HTTP-Redirect`   | TRUE     | TRUE     |           |           |
...
Endpoint for keys [JSON Web Key](https://tools.ietf.org/html/rfc7517).
...
| binding     | HTTP binding used to send request                 | `HTTP-POST` or `HTTP-Redirect`   | TRUE     | TRUE     |           |           |
