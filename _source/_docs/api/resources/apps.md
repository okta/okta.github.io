...
The current workaround is to manually configure the desired application via the Okta user interface in a preview (sandbox) organization and view the application via [Get Application](#get-application)

> App provisioning settings currently cannot be managed via the API and must be configured via the Okta user interface.

##### Features

Applications may support optional provisioning features on a per-app basis.

> Provisioning features currently may not be configured via the API and must be configured via the Okta user interface.

The list of provisioning features an app may support are:

|------------------------+------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| App Feature            | Name in the Okta User Interface          | Description                                                                                                                                                                                                                                    |
| ---------------------- | ---------------------- | -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
* If `autoKeyRotation` is not specified, the client automatically opts in for Okta's [key rotation](/authentication-guide/tokens/validating-id-tokens). This property may be updated via the API or via the Okta user interface.
...
3. If the background provisioning job completes with an error, the application user remains with the `STAGED` status but has `syncState` as `ERROR`.  A provisioning task is created in the Okta user interface that must be resolved to retry the job.
...
| ERROR        | Background provisioning job failed to update the user's profile in the target application. A provisioning task is created in the Okta user interface that must be resolved to retry the job. |
|--------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|

> User provisioning currently must be configured via the Okta user interface and is only available with specific editions.
...
Application User profiles are app-specific but may be customized by the Profile Editor in the Okta user interface. SSO apps typically don't support a user profile while apps with [user provisioning features](#features) have an app-specific profiles with optional and/or required properties.  Any profile properties visible in the Okta user interface for an application assignment can also be assigned via the API. Some properties are reference properties and imported from the target application and only allow specific values to be configured.
